...
class Grocery_CRUD_Generic_Model  extends Grocery_CRUD_Model  {
    	...
    	if(!empty($this->relation))
    	{
    		foreach($this->relation as $relation)
    		{
				...
				if(strstr($related_field_title,'{'))
				{
    				...
    				$select .= ", CONCAT('".str_replace(array('{','}'),array("',COALESCE(".$this->protect_identifiers($unique_join_name).".".$this->ESCAPE_CHAR, $this->ESCAPE_CHAR."::varchar, ''),'"),str_replace("'","\\'",$related_field_title))."') as ".$this->protect_identifiers($unique_field_name);
				}
    			else
    			{
    ...
    function get_relation_array($field_name , $related_table , $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
    {
    	...
    	if(strstr($related_field_title,'{'))
    	{
            ...
            /* Modificación para permitir campos enteros */
    		...
    		$select .= "CONCAT('".str_replace(array('{','}'),array("',COALESCE(".$this->ESCAPE_CHAR , $this->ESCAPE_CHAR."::varchar, ''),'"),str_replace("'","\\'", $related_field_title))."') as ".$this->protect_identifiers($field_name_hash);
    	}
    	else
    	{
