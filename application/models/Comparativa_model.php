...
//        pr($datos);
    ...
    function get_comparativa_region($id = null, $anio = 2016, $tipo_reporte = Self::PERFIL, $umae = 0, $tpunidad = null) {
        if (is_null($id)) {
            return false;
        }
        ...
        if (!is_null($tpunidad) && !empty($tpunidad)) {
            $where .= " and (t.id_tipo_unidad = " . $tpunidad . ") ";
        }
        ...
        sum(himp.cantidad_no_accesos) nunca_entraron,        
         case when (sum(himp.cantidad_alumnos_inscritos)-sum(himp.cantidad_no_accesos)) < 1 then 0 else
        trunc(sum(himp.cantidad_alumnos_certificados)/(sum(himp.cantidad_alumnos_inscritos)-sum(himp.cantidad_no_accesos))::float*100) end etm,
